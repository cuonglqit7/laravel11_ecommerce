...
@section('title', 'Chỉnh sửa - ' . $articleCategory->name)
@section('navbar')
...
@section('content')
    <div class="mx-auto bg-white p-3 rounded-lg shadow-md text-sm">
        ...
        <div class="bg-white p-6">
            <h2 class="text-xl font-bold mb-4">Chỉnh sửa - {{ $articleCategory->name }}</h2>
            <form action="{{ route('articleCategories.update', $articleCategory) }}" method="POST">
                @csrf
                @method('PUT')
                <!-- Tên danh mục -->
                <div class="mb-4">
                ...
                <div class="mb-4">
                    ...
                    <input type="number" id="position" name="position" value="{{ $articleCategory->position }}"
                        min="1" class="w-full p-2 border rounded focus:ring focus:ring-blue-300">
                    @error('position')
                        <p class="text-red-500 text-sm">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea id="description" name="description" rows="3"
                        class="w-full p-2 border rounded focus:ring focus:ring-blue-300">{{ $articleCategory->description }}</textarea>
                </div>
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <label class="flex items-center gap-1">
                            <input type="radio" name="status" value="1"
                                {{ $articleCategory->status == '1' ? 'checked' : '' }} class="h-4 w-4 text-blue-600">
                        ...
                        <label class="flex items-center gap-1">
                            <input type="radio" name="status" value="0"
                                {{ $articleCategory->status == '0' ? 'checked' : '' }} class="h-4 w-4 text-red-600">
