        ...
        <div class="flex items-center mb-3 justify-between">
            ...
            @can('role-create')
                <a href="{{ route('roles.create') }}"
                    class="bg-blue-500 text-white px-3 py-2 rounded hover:bg-blue-600 text-xs">Thêm quyền</a>
            @endcan

        </div>
        <hr class="h-px my-2 bg-gray-200 border-0 dark:bg-gray-700">
        <form action="{{ route('roles.index') }}" method="POST" class="flex flex-wrap justify-end items-center gap-2 my-4">
            @csrf
            @method('GET')
            <div>
                    ...
                    <tr class="border-t hover:bg-gray-50 transition-all duration-200">
                        ...
                        <td class="p-3">
                            <a href="{{ route('roles.show', $role->id) }}"
                                class="font-semibold text-blue-600 hover:underline">{{ $role->name }}</a>
                        </td>
                        <td class="p-3">
                            @can('role-edit')
                                <form action="{{ route('roles.toggleStatus', $role->id) }}" method="POST">
                                    ...
                                    <label class="inline-flex items-center cursor-pointer">
                                        <input type="checkbox" name="status" class="sr-only peer"
                                            {{ $role->status ? 'checked' : '' }} onchange="this.form.submit()">
...
@push('scripts')
    ...
    <script>
        document.getElementById('selectAll').addEventListener('click', function() {
            const checkboxes = document.querySelectorAll('input[name="selected_roles[]"]');
