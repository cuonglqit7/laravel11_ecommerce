...
@section('title', 'Chỉnh sửa khuyến mãi - ' . $discount->code)
@section('navbar')
...
@section('content')
    <div class="mx-auto bg-white p-3 rounded-lg shadow-md text-sm">
        ...
        <div class="bg-white p-6">
            <h2 class="text-xl font-bold mb-4">Chỉnh sửa mã {{ $discount->code }}</h2>
            <form action="{{ route('discounts.update', $discount->id) }}" method="POST">
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="code" value="{{ $discount->code }}" id="code"
                            placeholder="Nhập mã code khuyến mãi..."
                    ...
                    <div>
                        <label for="numper_usage" class="block text-sm font-medium text-gray-700">Số lần sử dụng của khách
                            hàng</label>
                        <input type="number" name="numper_usage" id="numper_usage" min="0"
                            value="{{ $discount->numper_usage }}"
                            class="block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                        @error('numper_usage')
                            <p class="text-red-500 text-sm">{{ $message }}</p>
                        @enderror
                    ...
                    <div>
                        ...
                        <select name="discount_type" id="discount_type"
                            class="block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                            <option value="Percentage" {{ $discount->discount_type == 'Percentage' ? 'selected' : '' }}>Phần
                                trăm</option>
                            <option value="Fixed Amount" {{ $discount->discount_type == 'Fixed Amount' ? 'selected' : '' }}>
                                Số
                                tiền cố định</option>
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <input type="number" step="10" name="discount_value" id="discount_value"
                            placeholder="Nhập giá trị..." value="{{ $discount->discount_value }}"
                            class="block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                    ...
                    <div class="flex gap-2">
                        ...
                        <div class="w-1/2">
                            ...
                            <input type="date" name="start_date" id="start_date" value="{{ $discount->start_date }}"
                        ...
                        <div class="w-1/2">
                            ...
                            <input type="date" name="end_date" id="end_date" value="{{ $discount->end_date }}"
                                class="block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-red-500 focus:border-red-500">
                            @error('end_date')
                                <p class="text-red-500 text-sm">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div>
                        ...
                        <select name="status" id="status"
                            class="block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">
                            <option value="active" {{ $discount->status != 'inactive' ? 'selected' : '' }}>Kích hoạt
                            </option>
                            <option value="inactive" {{ $discount->status == 'inactive' ? 'selected' : '' }}>Tạm ẩn
                            </option>
                        </select>
                    </div>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="description" id="description" rows="3" placeholder="Nhập mô tả cho khuyến mãi"
                            class="block w-full p-2 border border-gray-300 rounded-md shadow-sm focus:ring-blue-500 focus:border-blue-500">{{ $discount->description }}</textarea>
                    </div>
                </div>
