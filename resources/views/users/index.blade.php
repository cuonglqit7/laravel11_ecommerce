                ...
                <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                    <li aria-current="page">
                        <div class="flex items-center">
                            ...
                            <span class="ms-1 text-sm font-medium text-gray-500 md:ms-2 dark:text-gray-400">Danh sách người
                                dùng</span>
                        </div>
                    </li>
                </ol>
                    ...
                    <tr class="border-t">
                        ...
                        <td class="p-2">
                            @can('role-list')
                                <a href="{{ route('users.show', $user->id) }}"
                                    class="font-medium dark:text-blue-500 hover:underline">{{ $user->name }}</a>
                            @endcan
                        </td>
                        ...
                        <td class="p-2">
                            @foreach ($user->getRoleNames() as $roleName)
                                <span
                                    class="bg-blue-100 text-blue-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded-sm dark:bg-gray-700 dark:text-blue-400 border border-blue-400">{{ $roleName }}</span>
                            @endforeach
                        </td>
                        <td class="flex gap-1">
                            @can('role-edit')
                                <a href="{{ route('users.edit', $user->id) }}"><svg
